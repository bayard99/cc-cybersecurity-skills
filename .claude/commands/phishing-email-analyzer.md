...
description: Analyze a suspicious email for phishing indicators. Pass a path to a .eml file.
---
...
> I need a path to a .eml file. Export the email from your mail client and run:
> `/phishing-email-analyzer /path/to/email.eml`
...
```bash
python skills/phishing-email-analyzer/scripts/analyze_email.py --json "$ARGUMENTS"
```
